...
import { FloatInput, FloatSelect, ToggleGroup, ResultCard, StatGrid } from '../ui';
...
export function GarmentsCalc({ history, onAdd, onClear }:any) {
  const {t,lang}=useLang(); const g=t.garments;
  const [gender,setGender]=useState('male');
  const [chest,setChest]=useState(''); const [waist,setWaist]=useState('');
  const [hip,setHip]=useState(''); const [shoulder,setShoulder]=useState('');
  const [sleeve,setSleeve]=useState(''); const [height,setHeight]=useState('');
  ...
  return (
    <CalcShell accent={A_GAR}
      info={lang==='bn'?`• বুকের মাপ ইঞ্চিতে দিন (আবশ্যক)\n• কোমর, হিপ, কাঁধ ঐচ্ছিক\n• হিসাব চাপুন`:`• Enter chest measurement in inches (required)\n• Waist, hip, shoulder are optional\n• Press Calculate`}
      ...
      <div style={{background:'#1a0f05',border:'1px solid #7c2d12',borderRadius:12,padding:'12px 14px',marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:700,color:A_GAR,marginBottom:8,display:'flex',alignItems:'center',gap:6}}><FaInfoCircle size={11}/>{g.howTo}</div>
        {g.howToTips?.map((tip:string,i:number)=>(<div key={i} style={{fontSize:12,color:'#d97706',padding:'2px 0',display:'flex',gap:6}}><span style={{color:A_GAR,fontWeight:700,flexShrink:0}}>•</span><span>{tip}</span></div>))}
      </div>

      <FloatInput label={`${g.chest} *`} accent={A_GAR} type="number" placeholder="38" value={chest} onChange={(e:any)=>setChest(e.target.value)} hint={lang==='bn'?'* আবশ্যক':'* Required'}/>
      <FloatInput label={g.waist} accent={A_GAR} type="number" placeholder="32" value={waist} onChange={(e:any)=>setWaist(e.target.value)}/>
      <FloatInput label={g.hip} accent={A_GAR} type="number" placeholder="40" value={hip} onChange={(e:any)=>setHip(e.target.value)}/>
      <FloatInput label={g.shoulder} accent={A_GAR} type="number" placeholder="17" value={shoulder} onChange={(e:any)=>setShoulder(e.target.value)}/>
      <FloatInput label={g.sleeve} accent={A_GAR} type="number" placeholder="25" value={sleeve} onChange={(e:any)=>setSleeve(e.target.value)}/>
      <FloatInput label={g.height} accent={A_GAR} type="number" placeholder="66" value={height} onChange={(e:any)=>setHeight(e.target.value)}/>

      {result&&!result.error&&(
        <ResultCard accent={A_GAR}>
          <div style={{textAlign:'center',marginBottom:14}}>
            <div style={{fontSize:11,fontWeight:600,color:'#6b6780',marginBottom:4}}>{g.recommended}</div>
            <div style={{fontSize:'clamp(48px,12vw,64px)',fontWeight:900,color:A_GAR,lineHeight:1}}>{result.size}</div>
          </div>
          <StatGrid items={[
            [g.chest,`${chest}" / ${result.chestCm}cm`,A_GAR],
            [g.waist,waist?`${waist}" / ${result.waistCm}cm`:'—',A_GAR],
            [g.hip,hip?`${hip}" / ${result.hipCm}cm`:'—',A_GAR],
            [g.shoulder,shoulder?`${shoulder}"`:'—',A_GAR],
          ]} cols={2}/>
        </ResultCard>
      )}
...
export function GSizeCalc() {
  ...
  return (
    <div style={{width:'100%',height:'100%',background:'#0f0f14',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:"'Noto Serif Bengali','Outfit',sans-serif"}}>
      <div style={{flex:1,minHeight:0,overflowY:'auto',padding:'14px',scrollbarWidth:'thin'} as React.CSSProperties}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:16,width:'100%'}}>
          {[['shirt',g.shirt],['pants',g.pants],['shoes',g.shoes]].map(([v,l])=>(
            <button key={v} onClick={()=>setCat(v)} style={{flex:1,padding:'10px 6px',background:cat===v?A_GSIZE:'#1a1a1e',color:cat===v?'#fff':'#6b6780',border:`2px solid ${cat===v?A_GSIZE:'#2e2e38'}`,borderRadius:10,fontSize:12,fontWeight:700,fontFamily:'inherit',cursor:'pointer'}}>{l}</button>
          ))}
        </div>
        <div style={{overflowX:'auto',borderRadius:12,border:'1px solid #2e2e38'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:320}}>
            <thead>
              <tr style={{background:A_GSIZE}}>
                {(isShoes?['BD','US','UK','EU','Asia']:['Size','BD','US','UK','EU','Asia',lang==='bn'?'রেফ':'Ref']).map((c:string)=>(
            ...
            <tbody>
              {data.map((row:any,i:number)=>(
                <tr key={i} style={{background:i%2===0?'#1a1a1e':'#111116'}}>
                  {(isShoes?[row.bd,row.us,row.uk,row.eu,row.asia]:[row.s,row.bd,row.us,row.uk,row.eu,row.asia,row.ref]).map((v:string,j:number)=>(
                    <td key={j} style={{padding:'10px 8px',textAlign:'center',fontWeight:j===0?800:500,color:j===0?A_GSIZE:'#a0a0b0',borderBottom:'1px solid #2e2e38',whiteSpace:'nowrap'}}>{v}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        <div style={{marginTop:10,fontSize:11,color:'#3a3a48',textAlign:'center',lineHeight:1.6}}>
...
function convTemp(v:number,from:string,to:string){const c=from==='celsius'?v:from==='fahrenheit'?(v-32)*5/9:v-273.15;return to==='celsius'?c:to==='fahrenheit'?c*9/5+32:c+273.15;}
const CATS2=['length','weight','temperature','area','volume','speed'];
const CAT_ICONS:any={length:'📏',weight:'⚖️',temperature:'🌡️',area:'📐',volume:'🧊',speed:'💨'};

export function UnitCalc({ history, onAdd, onClear }:any) {
  const {t,lang}=useLang(); const u=t.unit;
  const [cat,setCat]=useState('length');
  const [fromU,setFromU]=useState('meter'); const [toU,setToU]=useState('kilometer');
  const [val,setVal]=useState(''); const [result,setResult]=useState<any>(null);
  ...
  return (
    <CalcShell accent={A_UNIT}
      info={lang==='bn'?`• বিভাগ বেছে নিন\n• মান দিন\n• একক বেছে নিন\n• হিসাব চাপুন`:`• Select category\n• Enter value\n• Select units\n• Press Calculate`}
      onCalc={calc} calcLabel={t.calculate}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:8,marginBottom:16}}>
        {CATS2.map(c=>(
          <button key={c} onClick={()=>{setCat(c);setFromU(UNITS[c].units[0]);setToU(UNITS[c].units[1]);setResult(null);setVal('');}} style={{padding:'10px 6px',background:cat===c?A_UNIT:'#1a1a1e',color:cat===c?'#fff':'#6b6780',border:`2px solid ${cat===c?A_UNIT:'#2e2e38'}`,borderRadius:10,fontSize:12,fontWeight:700,fontFamily:'inherit',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
      ...
      <FloatInput label={u.value} accent={A_UNIT} type="number" placeholder="1" value={val} onChange={(e:any)=>setVal(e.target.value)}/>

      <div style={{display:'grid',gridTemplateColumns:'1fr 36px 1fr',gap:8,alignItems:'center',marginBottom:16}}>
        <FloatSelect label={u.from} accent={A_UNIT} value={fromU} onChange={(e:any)=>setFromU(e.target.value)}>
        ...
        <FloatSelect label={u.to} accent={A_UNIT} value={toU} onChange={(e:any)=>setToU(e.target.value)}>
          {data.units.map((u2:string)=><option key={u2} value={u2}>{data.labels[u2]}</option>)}
        </FloatSelect>
      </div>
      ...
      {result!==null&&!result?.error&&(
        <ResultCard accent={A_UNIT}>
          <div style={{textAlign:'center'}}>
            <div style={{fontSize:13,color:'#6b6780',marginBottom:6}}>{val} {data.labels[fromU]}</div>
            <div style={{fontSize:'clamp(28px,7vw,38px)',fontWeight:900,color:A_UNIT,lineHeight:1.2,wordBreak:'break-word'}}>{result}</div>
            <div style={{fontSize:15,fontWeight:600,color:'#a0a0b0',marginTop:4}}>{data.labels[toU]}</div>
          </div>
        </ResultCard>
      )}
      {result?.error&&<div style={{color:'#ef4444',fontSize:14,marginTop:10,fontWeight:600}}>⚠️ {result.error}</div>}
